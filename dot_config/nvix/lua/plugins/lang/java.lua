-- Java (jdtls via nvim-jdtls). Reproduced from LazyVim's lang.java extra:
-- LazyVim.has -> Util.has, dropped `recommended` + the cmp-nvim-lsp capabilities
-- branch (we use blink). jdtls is started by nvim-jdtls per-buffer, not lspconfig.
if not require("config.profile").has("java") then
  return {}
end
...
return {
  { "mason-org/mason.nvim", opts = { ensure_installed = { "jdtls", "gradle-language-server" } } },
  {
    "nvim-treesitter/nvim-treesitter",
    opts = { ensure_installed = { "java" } },
  },

  -- java debugger + test packages (mason), and a remote-attach dap config
  {
  ...
  -- install jdtls via mason but let nvim-jdtls start it
  {
    ...
    opts = {
      ...
      setup = {
        jdtls = function()
          return true -- avoid duplicate servers (nvim-jdtls starts it)
        end,
      },
    },
  },
