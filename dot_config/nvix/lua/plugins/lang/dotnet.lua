-- .NET / C# / Razor — the whole stack in one file (per the one-place model):
-- Roslyn LSP + test runner + debugger (easy-dotnet), the LazyDotnet TUI toggle,
-- profile-driven parsers + mason tools. The C# xmldoc autosnippet lives in
-- luasnip.lua (also dotnet-gated). css/html/sass servers (for Razor markup)
-- auto-enable via mason-lspconfig once installed.
if not require("config.profile").has("dotnet") then
  return {}
end

return {
  -- profile-driven mason tools (Roslyn is fetched by easy-dotnet itself, not Mason)
  {
  ...
  -- c_sharp/razor parsers have no other provider (declared only here)
  {
    "nvim-treesitter/nvim-treesitter",
    opts = { ensure_installed = { "c_sharp", "razor" } },
  },

  -- ── easy-dotnet: Roslyn LSP, test runner, debugger, secrets, package mgmt ──
